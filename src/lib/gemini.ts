...
export interface ScanSheetMusicResult {
  xml: string;
  source: 'gemini' | 'demo';
}

export async function scanSheetMusic({ base64Data, mimeType, systemPrompt }: ScanSheetMusicOptions): Promise<ScanSheetMusicResult> {
  const apiKey = process.env.GEMINI_API_KEY;

  if (!apiKey) {
    console.warn("GEMINI_API_KEY is not defined. Returning demo sheet with transparent source marking.");
    return {
      xml: getStandardDrumMusicXML("Scannet Trommenode (Demo)", 110, "standard"),
      source: "demo",
    };
  }
